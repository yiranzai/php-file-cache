...
class FilesystemTest extends TestCase
{
    public function testFile()
    {
        $file      = new Filesystem();
        $str       = 'test';
        $path      = __DIR__ . DIRECTORY_SEPARATOR . 'testOne.txt';
        $pathTwo   = __DIR__ . DIRECTORY_SEPARATOR . 'testTwo.yaml';
        $pathThree = __DIR__ . DIRECTORY_SEPARATOR . 'testThree.ini';
    ...
    public function testDirTwo()
    {
        ...
        $this->assertTrue($file->exists($path));
        $this->assertTrue($file->exists($pathTwo));
        $this->assertSame(substr(sprintf('%o', fileperms($path)), -4), $file->chmod($path));
